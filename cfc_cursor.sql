...
procedure create_pim(AKT_EV VARCHAR2, SZEKT VARCHAR2, ALSZEKT VARCHAR2, ESZKOZ VARCHAR2, v_inv2_table VARCHAR2, v_lifetime_table VARCHAR2, v_arindex_1999 VARCHAR2, v_szektor VARCHAR2, v_eszkozcsp VARCHAR2, v_alszektor VARCHAR2, gfcf_year VARCHAR2);
procedure sector_unite(AKT_EV VARCHAR2, ESZKOZ VARCHAR2, v_szektor VARCHAR2, v_alszektor VARCHAR2, v_eszkozcsp VARCHAR2);
...
sql_statement := 'SELECT COUNT(*) FROM PKD.'|| v_inv2_table ||' WHERE ALSZEKTOR = '''|| v_out_alszektor(x) ||''' AND ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' ';
...
open tev_phad_cur for tev_phad_sql_text;
        ...
        loop
            ...
            begin
				...
				--LIFETIME:
				execute immediate 'SELECT Y'|| l_cell(l_idx) ||' FROM '|| v_out_schema ||'.'|| v_lifetime_table ||' WHERE ALSZEKTOR = '''|| tev_phad_rec.ALSZEKTOR ||''' 
				AND ESZKOZCSP = '''|| tev_phad_rec.ESZKOZCSP ||''' AND AGAZAT = '''|| tev_phad_rec.AGAZAT ||''' ' INTO lt_rows;	
				...
				--INV2:
				execute immediate 'SELECT Y'|| l_cell(l_idx) ||' FROM '|| v_out_schema ||'.'|| v_inv2_table ||'	WHERE ALSZEKTOR = '''|| tev_phad_rec.ALSZEKTOR ||''' 
				AND ESZKOZCSP = '''|| tev_phad_rec.ESZKOZCSP ||''' AND AGAZAT = '''|| tev_phad_rec.AGAZAT ||''' ' INTO inv2_rows;
