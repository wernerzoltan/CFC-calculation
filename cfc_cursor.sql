...
procedure create_pim(AKT_EV VARCHAR2, SZEKT VARCHAR2, ALSZEKT VARCHAR2, ESZKOZ VARCHAR2, v_inv2_table VARCHAR2, v_lifetime_table VARCHAR2, v_arindex_1999 VARCHAR2, v_szektor VARCHAR2, v_eszkozcsp VARCHAR2, v_alszektor VARCHAR2, gfcf_year VARCHAR2, v_out_schema VARCHAR2);
procedure sector_unite(AKT_EV VARCHAR2, ESZKOZ VARCHAR2, v_szektor VARCHAR2, v_alszektor VARCHAR2, v_eszkozcsp VARCHAR2, v_out_schema VARCHAR2);
...
sql_statement := 'SELECT COUNT(*) FROM PKD19.'|| v_inv2_table ||' WHERE ALSZEKTOR = '''|| v_out_alszektor(x) ||''' AND ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' ';
...
act_minus := ''|| AKT_EV ||'' - 1; -- folyó ár kiszámításánál szükséges
	...
	sql_statement := 'SELECT AGAZAT FROM PKD19.'|| v_inv2_table ||' WHERE ALSZEKTOR = '''|| v_out_alszektor(x) ||''' AND ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' '; 
	...
	VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Create '|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'', '');
		...
		SELECT COUNT(*) INTO v FROM all_tables where table_name = UPPER(''|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'');
		IF v = 1 THEN
		
			EXECUTE IMMEDIATE'
			DELETE FROM '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' 
			'
			;
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate PIM', '');
		...
		-- INV2 táblából átvesszük az adatokat
			...
			EXECUTE IMMEDIATE'
				...
				WHERE AGAZAT = '|| V_AGAZAT_IMP(l) ||' AND ALSZEKTOR = '''|| v_out_alszektor(x) ||''' AND ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' 
				'
				;	
...
tev_phad_sql_text := 'SELECT rowid sorid, ALSZEKTOR, ESZKOZCSP, AGAZAT from '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' WHERE ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' AND ALSZEKTOR = '''|| v_out_alszektor(x) ||''' ';

v_ev :=  ''|| AKT_EV ||'' - l_cell(l_idx);
...
open tev_phad_cur for tev_phad_sql_text;
        ...
        loop
            ...
            begin
				...
				--LIFETIME:
				execute immediate 'SELECT Y'|| l_cell(l_idx) ||' FROM '|| v_out_schema ||'.'|| v_lifetime_table ||' WHERE AGAZAT = '''|| tev_phad_rec.AGAZAT ||''' AND ALSZEKTOR = '''|| tev_phad_rec.ALSZEKTOR ||''' AND ESZKOZCSP = '''|| tev_phad_rec.ESZKOZCSP ||'''' INTO lt_rows;	
				
				IF lt_rows IS NULL THEN
				...
				--PIM:
				sql_statement := 'SELECT COUNT(*) FROM '|| v_out_schema ||'.'|| V_PIM_INPUT(a) ||' WHERE EV = '|| v_ev ||' AND LIFETIME = '|| lt_rows ||' ';
				EXECUTE IMMEDIATE sql_statement INTO v;
				
					IF v > 0 THEN
				
						execute immediate 'SELECT ERTEK FROM '|| v_out_schema ||'.'|| V_PIM_INPUT(a) ||' WHERE EV = '|| v_ev ||' AND LIFETIME = '|| lt_rows ||' ' INTO pim_rows;
				...
				--INV2:
				execute immediate 'SELECT Y'|| l_cell(l_idx) ||' FROM '|| v_out_schema ||'.'|| v_inv2_table ||'	WHERE AGAZAT = '''|| tev_phad_rec.AGAZAT ||''' AND ALSZEKTOR = '''|| tev_phad_rec.ALSZEKTOR ||''' AND ESZKOZCSP = '''|| tev_phad_rec.ESZKOZCSP ||''' ' INTO inv2_rows;
					
                execute immediate '
				UPDATE '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' 
                set Y'|| l_cell(l_idx) ||' = :sum_all
				WHERE rowid=:sorid
				  ' 
				using 
				inv2_rows * pim_rows, --SUM_ALL
...
open tev_phad_cur for tev_phad_sql_text;
        ...
        loop
            ...
            begin
				...
				execute immediate 'select '|| v_pim_sum ||' FROM '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' WHERE AGAZAT = '|| tev_phad_rec.AGAZAT ||' ' into v_sum;
			...
			CASE WHEN ''|| v_pim(a) ||'' = 'net' OR ''|| v_pim(a) ||'' =  'grs' THEN  
			 ...
			 WHEN ''|| v_pim(a) ||'' = 'cfc' THEN
				...
				EXECUTE IMMEDIATE'
					...
					('|| v_pim_create_out_cfc ||' FROM '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' WHERE OUTPUT = ''cfc''
					'
					;				
					
				PKD19.drop_table(''|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'');
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate actual price', '');		
	...
	sum_1999 := 'a.YSUM_99 * b.Y'|| AKT_EV ||'_1999'; -- folyó ár kiszámítása
	sum_unch := 'a.YSUM_99 * b.Y'|| act_minus ||'_1999'; -- változatlan ár kiszámítása
...
open tev_phad_cur for tev_phad_sql_text;
        ...
        loop
            ...
            begin
				...
				execute immediate 'SELECT '|| sum_1999 ||' FROM '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' a
				INNER JOIN '|| v_out_schema ||'.'|| v_arindex_1999 ||' b
				ON a.AGAZAT = b. AGAZAT
				WHERE a.AGAZAT = '|| tev_phad_rec.AGAZAT ||'
				AND b.ALSZEKTOR = '''|| v_out_alszektor(x) ||''' AND b.ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' ' into v_sum_akt;
	
                execute immediate '
				UPDATE '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' 
                set YSUM_F = :v_sum_akt
				WHERE rowid=:sorid
				  ' 
				using 
				v_sum_akt, --v_sum_akt
...
open tev_phad_cur for tev_phad_sql_text;
        ...
        loop
            ...
            begin
				...
				execute immediate 'SELECT '|| sum_unch ||' FROM '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' a
				INNER JOIN '|| v_out_schema ||'.'|| v_arindex_1999 ||' b
				ON a.AGAZAT = b. AGAZAT
				WHERE a.AGAZAT = '|| tev_phad_rec.AGAZAT ||'
				AND b.ALSZEKTOR = '''|| v_out_alszektor(x) ||''' AND b.ESZKOZCSP = '''|| v_out_eszkozcsp(b) ||''' ' into v_sum_unch;
	
                execute immediate '
				UPDATE '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' 
                set YSUM_V = :v_sum_unch
				WHERE rowid=:sorid
				  ' 
				using 
				v_sum_unch, --v_sum_akt
				tev_phad_rec.row_id --sorid
				;				  
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate price SUM', '');
	...
	EXECUTE IMMEDIATE'
		INSERT INTO '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
		(output, szektor, ALSZEKTOR, eszkozcsp, agazat, YSUM_99, YSUM_F, YSUM_V)
		SELECT '''|| v_pim(a) ||''' , '''|| v_out_szektor(x) ||''', '''|| v_out_alszektor(x) ||''', '''|| v_out_eszkozcsp(b) ||''', ''SUM'', sum(YSUM_99), sum(YSUM_F), sum(YSUM_V)
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Create output table', '');
	...
	EXECUTE IMMEDIATE'
	DELETE FROM '|| v_out_schema ||'.'|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_'|| AKT_EV ||'
	'
	;
...
PKD19.drop_table(''|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_CFC_'|| AKT_EV ||'');
PKD19.drop_table(''|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_GRS_'|| AKT_EV ||'');
PKD19.drop_table(''|| v_out_szektor(x) ||'_'|| v_out_alszektor(x) ||'_'|| v_out_name(b) ||'_NET_'|| AKT_EV ||'');
...
procedure sector_unite(AKT_EV VARCHAR2, ESZKOZ VARCHAR2, v_szektor VARCHAR2, v_alszektor VARCHAR2, v_eszkozcsp VARCHAR2, v_out_schema VARCHAR2) AS 
v NUMERIC;
