...
create or replace PACKAGE CFC_OUTPUT_NEW AUTHID CURRENT_USER AS 
...
TYPE t_update_eszkcsp IS TABLE OF VARCHAR2(50 CHAR) INDEX BY PLS_INTEGER;
...
procedure CFC_OUTPUT_NEW(SZEKTOR VARCHAR2, ALSZEKTOR VARCHAR2, EVSZAM VARCHAR2);
procedure CFC_OUTPUT_NEW_MOD;

END CFC_OUTPUT_NEW;

*/
create or replace PACKAGE BODY CFC_OUTPUT_NEW AS 
F_V_99 T_V_99;
F_V_99a T_V_99a;
...
v_update_eszkcsp t_update_eszkcsp;
...
v_out_table VARCHAR2(50) := 'C_OUTPUT';
v_out_table_all VARCHAR2(50) := 'C_OUTPUT_3JEGYU';
v_out_table_mod VARCHAR2(50) := 'C_OUTPUT_FINAL';

procedure CFC_OUTPUT_NEW(SZEKTOR VARCHAR2, ALSZEKTOR VARCHAR2, EVSZAM VARCHAR2) AS
...
-- 6.lépés: nem klasszikus eszközök importálása
...
FOR x IN 13..23 LOOP --2007 - 2017 között
...
FOR s IN v_szektorok.FIRST..v_szektorok.LAST LOOP
                ...
                IF ''|| v_szektorok(s) ||'' = 'S1313' THEN
                               ...
                               -- WHERE a.AGAZAT = ''39'' AND a.ALSZEKTOR = ''S1313'' AND a.CFC_NET_GRS = ''cfc'' AND a.EV = ''2018'' AND a.F_V_99 = ''F'') - 5
                               -- WHERE AGAZAT = ''39'' AND ALSZEKTOR = ''S1313'' AND CFC_NET_GRS = ''cfc'' AND EV = ''2018'' AND F_V_99 = ''F''
                               -- '
                               -- ;
                               ...
                               -- SET FOLDJAVITAS = 
                               -- (SELECT ROUND(NVL(FOLDJAVITAS, 0)) FROM '|| v_out_table ||' a
                               -- WHERE a.AGAZAT = ''84'' AND a.ALSZEKTOR = ''S1313'' AND a.CFC_NET_GRS = ''cfc'' AND a.EV = ''2018'' AND a.F_V_99 = ''F'') + 5
                               -- WHERE AGAZAT = ''84'' AND ALSZEKTOR = ''S1313'' AND CFC_NET_GRS = ''cfc'' AND EV = ''2018'' AND F_V_99 = ''F''
                               -- '
                               -- ;
                               ...
                               -- SET FOLDJAVITAS = 
                               -- (SELECT ROUND(NVL(FOLDJAVITAS, 0)) FROM '|| v_out_table ||' a
                               -- WHERE a.AGAZAT = ''39'' AND a.ALSZEKTOR = ''S1313'' AND a.CFC_NET_GRS = ''cfc'' AND a.EV = ''2018'' AND a.F_V_99 = ''V'') - 5
                               -- WHERE AGAZAT = ''39'' AND ALSZEKTOR = ''S1313'' AND CFC_NET_GRS = ''cfc'' AND EV = ''2018'' AND F_V_99 = ''V''
                               -- '
                               -- ;							   
                               ...
                               -- SET FOLDJAVITAS = 
                               -- (SELECT ROUND(NVL(FOLDJAVITAS, 0)) FROM '|| v_out_table ||' a
                               -- WHERE a.AGAZAT = ''84'' AND a.ALSZEKTOR = ''S1313'' AND a.CFC_NET_GRS = ''cfc'' AND a.EV = ''2018'' AND a.F_V_99 = ''V'') + 4
                               -- WHERE AGAZAT = ''84'' AND ALSZEKTOR = ''S1313'' AND CFC_NET_GRS = ''cfc'' AND EV = ''2018'' AND F_V_99 = ''V''
                               -- '
                               -- ;
...
-- 7. lépés: CFC_IMPUTALT tábla értékeit hozzáadjuk a meglévő értékekhez

dbms_output.put_line('7. lépés START: ' || systimestamp);
...
FOR x IN v_out_cfc.FIRST..v_out_cfc.LAST LOOP
				...
				-- DBMS_OUTPUT.PUT_LINE(''|| v_in_year(b) ||'');
				-- DBMS_OUTPUT.PUT_LINE(''|| v_update ||'');
				-- DBMS_OUTPUT.PUT_LINE(''|| v_out_nk_up_c(c) ||'');
				-- DBMS_OUTPUT.PUT_LINE(''|| F_V_99(d) ||'');
				-- DBMS_OUTPUT.PUT_LINE(''|| v_szektorok(s) ||'');
...
IF ''|| v_out_eszkcsp_c(c) ||'' IN ('AN112', 'AN1131', 'AN1139t', 'AN1173s') AND ''|| v_szektorok(s) ||'' IN ('S11', 'S12')  -- ÉPÜLET, JÁRMű, TARTÓSGÉP, SZOFTVER: S11, S12
...
dbms_output.put_line('7. lépés STOP: ' || systimestamp);


-- 8. lépés: 3 jegyű ágazatok összevonása

dbms_output.put_line('8. lépés START: ' || systimestamp);
...
FOR z IN v_out_cfc.FIRST..v_out_cfc.LAST LOOP
	...
	-- S1311a esetén 2018-tól 723->72 K+F esetén
	ELSIF ''|| v_szektorok(s) ||'' = 'S1311a' THEN

	FOR x IN 24..v_in_year.COUNT LOOP --2018-tól
...
dbms_output.put_line('8. lépés STOP: ' || systimestamp);



-- 9. lépés: SUM rekord újraszámítása nem klasszikusok esetében
...
-- 10. lépés: CFC_LAKAS_MEZO tábla értékeit beillesztjük
...
-- 12.lépés: SUM_ALL mező feltöltése
...
PROCEDURE CFC_OUTPUT_NEW_MOD AS 
...
FOR c IN v_update_eszkcsp.FIRST..v_update_eszkcsp.LAST LOOP

FOR x IN 2..v_out_cfc.LAST LOOP -- CFC nem kell
...
-- klasszikus eszközcsoportok
v_update_eszkcsp(1) := 'EPULET'; --AN112
v_update_eszkcsp(2) := 'GYORSGEP'; --AN1139g
v_update_eszkcsp(3) := 'TARTOSGEP'; --AN1139t
v_update_eszkcsp(4) := 'JARMU'; --AN1131
v_update_eszkcsp(5) := 'SZOFTVER'; -- AN1173s
v_update_eszkcsp(6) := 'ORIGINALS'; --AN1174
v_update_eszkcsp(7) := 'FOLDJAVITAS'; --AN1123
v_update_eszkcsp(8) := 'K_F'; --AN1171
v_update_eszkcsp(9) := 'FEGYVER'; --AN114
v_update_eszkcsp(10) := 'OWNSOFT'; --AN1173o
v_update_eszkcsp(11) := 'NOE6'; 
v_update_eszkcsp(12) := 'KISERTEKU'; 
v_update_eszkcsp(13) := 'WIZZ'; 
v_update_eszkcsp(14) := 'TCF'; 
v_update_eszkcsp(15) := 'EGYEB_ORIG'; 
v_update_eszkcsp(16) := 'LAKAS'; 
v_update_eszkcsp(17) := 'BESOROLT_VALL'; 
