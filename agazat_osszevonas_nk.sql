...
BEGIN					
		...
		SET '|| v_out_nk_eszkcsp ||' = 
		(SELECT ROUND(SUM(NVL('|| v_out_nk_eszkcsp ||',0))) FROM PKD19.'|| v_out_table ||' a
		WHERE a.AGAZAT IN ('|| agazatok_sum ||') AND a.ALSZEKTOR = '''|| v_szektorok ||''' AND a.CFC_NET_GRS = '''|| v_out_cfc ||''' AND EV = '''|| v_in_year ||''' AND a.F_V_99 = '''|| F_V_99 ||''')
