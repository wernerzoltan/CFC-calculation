...
SELECT COUNT(*) INTO z FROM user_tab_cols WHERE table_name = ''|| v_out_table ||'';
  ...
  END IF;
	...
	sql_statement := 'SELECT * FROM PKD19.'|| v_rate_table ||' WHERE SZEKTOR = '''|| v_szektor ||''' AND ALSZEKTOR = '''|| v_alszektor ||''' ';
	EXECUTE IMMEDIATE sql_statement BULK COLLECT INTO v_rate;
	
	sql_statement := 'SELECT DISTINCT T08 FROM PKD19.'|| v_rate_table ||' WHERE SZEKTOR = '''|| v_szektor ||''' AND ALSZEKTOR = '''|| v_alszektor ||''' '; 
...
-- 1. lépés az új táblába tesszük a T08/T03-as ágazatok listáját
		...
		FOR c IN v_rate_d.FIRST..v_rate_d.LAST LOOP	
			...
			EXECUTE IMMEDIATE'
			INSERT INTO PKD19.'|| v_out_table ||' (SZEKTOR, ALSZEKTOR, AGAZAT) VALUES 
  	...
  	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''01'' WHERE AGAZAT = ''1'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''02'' WHERE AGAZAT = ''2'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''03'' WHERE AGAZAT = ''3'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''04'' WHERE AGAZAT = ''4'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''05'' WHERE AGAZAT = ''5'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''06'' WHERE AGAZAT = ''6'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''07'' WHERE AGAZAT = ''7'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''08'' WHERE AGAZAT = ''8'' ';
	EXECUTE IMMEDIATE'
	UPDATE PKD19.'|| v_out_table ||' SET AGAZAT = ''09'' WHERE AGAZAT = ''9'' ';
...
PKD19.TRUNCATE_TABLE(''|| rate_temp ||'');
	...
	FOR c IN v_rate_d.FIRST..v_rate_d.LAST LOOP      	 
		...
		SELECT T08, T03 from PKD19.'|| v_rate_table ||'
	...
	FOR c IN v_rate.FIRST..v_rate.LAST LOOP
		...
		FROM PKD19.'|| v_in_table ||' a
		INNER JOIN PKD19.'|| v_rate_table ||' b
		ON a.T03 = b.T03
