...
PROCEDURE create_pim(AKT_EV VARCHAR2, SZEKT VARCHAR2, ALSZEKT VARCHAR2, ESZKOZ VARCHAR2, v_inv2_table VARCHAR2, v_lifetime_table VARCHAR2, v_arindex_1999 VARCHAR2, v_szektor VARCHAR2, v_eszkozcsp VARCHAR2, v_alszektor VARCHAR2) AS
...
TYPE t_agazat_type IS TABLE OF INV2_ALL_T08_180904%ROWTYPE; 
...
-- associative array-ba áttesszük a INV2_xx tábla sorait, ahonnan majd az ágazatok listáját fogjuk felhasználni
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'STARTING', '');
	...
	FOR a IN 1..v_pim.COUNT LOOP 
	 ...
	 -- log
INSERT INTO logging (created_on, info, proc_name, message, backtrace)
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Create '|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'', '');
	
		EXECUTE IMMEDIATE'
		CREATE TABLE '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' 
		'|| v_pim_create ||'
		'
		;
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate PIM', '');
		...
		-- INV2 táblából átvesszük az adatokat
			...
			EXECUTE IMMEDIATE'
				INSERT INTO '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
				(OUTPUT, SZEKTOR, ALSZEKTOR1, ESZKOZCSP, AGAZAT, ALAGAZAT)
				SELECT '''|| v_pim(a) ||''' as OUTPUT, SZEKTOR, ALSZEKTOR1, ESZKOZCSP, AGAZAT, ALAGAZAT
				FROM '|| v_out_schema ||'.'|| v_inv2_table ||'
				WHERE SZEKTOR = '''|| v_szektor ||''' AND ESZKOZCSP = '''|| v_eszkozcsp ||''' AND ALSZEKTOR1 = '''|| v_alszektor ||''' AND AGAZAT = '|| V_AGAZAT_IMP(l) ||'
				'
				;		
			...
			WHILE l_idx IS NOT NULL LOOP
				...
				EXECUTE IMMEDIATE'
				UPDATE '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
				SET Y'|| l_cell(l_idx) ||' = (SELECT (Y'|| l_cell(l_idx) ||' * (SELECT ERTEK FROM '|| V_PIM_INPUT(a) ||'
				WHERE EV = ('|| AKT_EV ||' - '|| l_cell(l_idx) ||') 
				AND LIFETIME = (SELECT Y'|| l_cell(l_idx) ||' FROM '|| v_out_schema ||'.'|| v_lifetime_table ||'
				WHERE AGAZAT = '|| V_AGAZAT_IMP(l) ||' AND SZEKTOR = '''|| v_szektor ||''' AND ALSZEKTOR1 = '''|| v_alszektor ||''' AND ESZKOZCSP = '''|| v_eszkozcsp ||''' )))
				FROM '|| v_out_schema ||'.'|| v_inv2_table ||'
				WHERE AGAZAT = '|| V_AGAZAT_IMP(l) ||' AND SZEKTOR = '''|| v_szektor ||''' AND ALSZEKTOR1 = '''|| v_alszektor ||''' AND ESZKOZCSP = '''|| v_eszkozcsp ||''')
				WHERE SZEKTOR = '''|| v_szektor ||''' AND ALSZEKTOR1 = '''|| v_alszektor ||''' AND ESZKOZCSP = '''|| v_eszkozcsp ||''' AND AGAZAT = '|| V_AGAZAT_IMP(l) ||'
				'
				;
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate SUM', '');
		...
		FOR n IN 1..V_AGAZAT_IMP.COUNT LOOP  
			...
			UPDATE '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
			SET YSUM = 
			(SELECT '|| v_pim_sum ||' FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
		...
		--3. lépés: létrehozunk egy átmeneti táblát, ahol 1 évvel eltoljuk a CFC értékeket
			...
			/*	
				EXECUTE IMMEDIATE'
					CREATE TABLE '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'_2
					'|| v_pim_create ||'
					'
					;
				*/
				...
				/*
				EXECUTE IMMEDIATE'
					INSERT INTO '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'_2
					('|| v_pim_create_out_cfc ||' FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
					'
					;
					*/
				...
				PKD.drop_table(''|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'');

				/*
				EXECUTE IMMEDIATE'
					ALTER TABLE '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'_2
					RENAME TO '|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
					'
					;
				*/
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate actual price', '');		
	...
	sum_unch := 'a.YSUM * b.Y'|| act_minus ||'_1999'; -- változatlan ár kiszámítása
		...
		FOR o IN 1..V_AGAZAT_IMP.COUNT LOOP 
				...
				FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||' a
				INNER JOIN '|| v_out_schema ||'.'|| v_arindex_1999 ||' b
				ON a.AGAZAT = b. AGAZAT
				WHERE a.AGAZAT = '|| V_AGAZAT_IMP(o) ||'
				AND b.SZEKTOR = '''|| v_szektor ||''' AND b.ALSZEKTOR1 = '''|| v_alszektor ||''' AND b.ESZKOZCSP = '''|| v_eszkozcsp ||''')
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Calculate price SUM', '');
	...
	EXECUTE IMMEDIATE'
		INSERT INTO '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
		(output, szektor, alszektor1, eszkozcsp, agazat, YSUM, YSUM_AKT, YSUM_UNCH)
		SELECT '''|| v_pim(a) ||''' , '''|| v_szektor ||''', '''|| v_alszektor ||''', '''|| v_eszkozcsp ||''', ''SUM'', sum(YSUM), sum(YSUM_AKT), sum(YSUM_UNCH)
		FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| v_pim(a) ||'_'|| AKT_EV ||'
		WHERE SZEKTOR = '''|| v_szektor ||''' AND ALSZEKTOR1 = '''|| v_alszektor ||''' AND ESZKOZCSP = '''|| v_eszkozcsp ||'''
...
VALUES (TO_CHAR(CURRENT_TIMESTAMP, 'YYYY.MM.DD HH24:MI:SS.FF'), 'Info', ''|| procName ||'', 'Create output table', '');
	...
	EXECUTE IMMEDIATE'
		CREATE TABLE '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'
		'|| v_pim_create ||'
		'
		;

	EXECUTE IMMEDIATE'
		INSERT INTO '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'
		('|| v_pim_create_out ||' FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_CFC_'|| AKT_EV ||'
		'
		;
	...
	EXECUTE IMMEDIATE'
		INSERT INTO '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'
		('|| v_pim_create_out ||' FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_NET_'|| AKT_EV ||'
		'
		;
	...
	EXECUTE IMMEDIATE'
		INSERT INTO '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'
		('|| v_pim_create_out ||'	FROM '|| v_out_schema ||'.'|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_GRS_'|| AKT_EV ||'
		'
		;
...
PKD.drop_table(''|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_CFC_'|| AKT_EV ||'');
PKD.drop_table(''|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_GRS_'|| AKT_EV ||'');
PKD.drop_table(''|| SZEKT ||'_'|| ALSZEKT ||'_'|| ESZKOZ ||'_NET_'|| AKT_EV ||'');
...
procedure sector_unite(AKT_EV VARCHAR2, ESZKOZ VARCHAR2, ALSZEKTOR VARCHAR2) AS 
v NUMERIC;

BEGIN

	FOR a IN 1..v_szektor_list.COUNT LOOP 
		
			
		SELECT COUNT(*) INTO v FROM all_tables where table_name = UPPER(''|| v_szektor_list(a) ||'_'|| ALSZEKTOR ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'');
		IF v=1 THEN
		
		EXECUTE IMMEDIATE'
		INSERT INTO '|| v_out_schema ||'.'|| ESZKOZ ||'_'|| AKT_EV ||'
		('|| v_pim_create_out ||' FROM '|| v_out_schema ||'.'|| v_szektor_list(a) ||'_'|| ALSZEKTOR ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'
		'
		;
		
		EXECUTE IMMEDIATE'
		DROP TABLE '|| v_out_schema ||'.'|| v_szektor_list(a) ||'_'||  ALSZEKTOR ||'_'|| ESZKOZ ||'_'|| AKT_EV ||'
		'
		;
...
v_szektor_list(1) := 'npvall'; 		-- S11
v_szektor_list(2) := 'vall'; 		-- S12
v_szektor_list(3) := 'korm';  		-- S13
v_szektor_list(4) := 'hazt';  		-- S14
v_szektor_list(5) := 'np';	 		-- S15
